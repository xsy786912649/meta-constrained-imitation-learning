...
test_file_name_list=["../../ref_traj/"+i+"_reftraj.mat" for i in [chr(x) for x in range(ord('Y'), ord('Z') + 1)]]
...
for i in range(len(t_data_test_list)):

    model = torch.load('model_meta.pkl') 
    model = model.to(device)

    data_loader_train = torch.utils.data.DataLoader(TensorDataset(torch.tensor(t_data_test_list[i]).float().requires_grad_(),torch.tensor(y_data_test_list[i]).float(),torch.tensor(sigma_data_test_list[i]).float()),shuffle = True, batch_size = batch_size_K)
    data_loader_test = torch.utils.data.DataLoader(TensorDataset(torch.tensor(t_data_test_list[i]).float().requires_grad_(),torch.tensor(y_data_test_list[i]).float(),torch.tensor(sigma_data_test_list[i]).float()),shuffle = False, batch_size = 400)
