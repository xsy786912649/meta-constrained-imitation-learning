...
filename_list=["./ref_traj/"+i+"_reftraj.mat" for i in [chr(x) for x in range(ord('A'), ord('Z') + 1)]]
test_file_name_list=["./ref_traj/"+i+"_reftraj.mat" for i in [chr(x) for x in range(ord('A'), ord('W') + 1)]]
...
for epoch in range(n_epochs):
    ...
    for i in range(len(t_data_list_thisepoch)):
        data_loader_train = torch.utils.data.DataLoader(TensorDataset(torch.tensor(t_data_list_thisepoch[i]).float(),torch.tensor(y_data_list_thisepoch[i]).float(),torch.tensor(sigma_data_list_thisepoch[i]).float()),shuffle = True, batch_size = batch_size_K)
        data_loader_train_list.append(data_loader_train)
        data_loader_train1 = torch.utils.data.DataLoader(TensorDataset(torch.tensor(t_data_list_thisepoch[i]).float(),torch.tensor(y_data_list_thisepoch[i]).float(),torch.tensor(sigma_data_list_thisepoch[i]).float()),shuffle = True, batch_size = batch_size_outer)
    ...
    if (epoch>50):
        ...
        optimizer=optimizer0
        #adjust_learning_rate(optimizer,epoch,0.00003)
    elif (epoch>150):
        optimizer=optimizer1
        #adjust_learning_rate(optimizer,epoch,0.00001)
