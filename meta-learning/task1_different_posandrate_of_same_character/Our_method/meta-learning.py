...
def run_for_each_round(model, round):
    filename_list=filename_list_whole[0:round]
    test_file_name_list=filename_list_whole[round:round+1]
    whole_task_num=len(filename_list) 
    task_test_num=1
    tranlation_list_train=translation_list_whole[0:round,:]
    tranlation_list_test=translation_list_whole[round:round+1,:]
    ...
    for ii in range(len(filename_list)):
        ...
        y_data=[]
        sigma_data=[]
        file_data=scio.loadmat(filename_list[ii])['refTraj'][0]
        for data in file_data:
            t_data.append([data[0][0][0]-1.0])
            y_data.append([data[1][0][0]+tranlation_list_train[ii][0],data[1][1][0]+tranlation_list_train[ii][1],data[1][2][0],data[1][3][0]])
    ...
    for i in range(len(test_file_name_list)):
        t_data=[]
        y_data=[]
        sigma_data=[]
        file_data=scio.loadmat(test_file_name_list[i])['refTraj'][0]
        for data in file_data:
            t_data.append([data[0][0][0]-1.0])
            y_data.append([data[1][0][0]+tranlation_list_test[i][0],data[1][1][0]+tranlation_list_test[i][1],data[1][2][0],data[1][3][0]])
            sigma_data.append(data[2]+0.001*np.identity(4))
    ...
    for epoch in range(n_epochs):
        ...
        tranlation_list_train_thisepoch=[]
        for i in number_list:
            ...
            tranlation_list_train_thisepoch.append(tranlation_list_train[i]) 
        ...
        tranlation_list_train_thisepoch.extend(tranlation_list_test)
            ...
            for number, data_loader_train_now in enumerate(data_train_now):
                ...
                if number < task_num + task_test_num:
                    ...
                    def loss_train_call(params, hparams):
                        return my_mse_loss(model(features, params), labels, sigmas)+bias_reg(params,hparams)+ constraint_voilations( model(features_constraint, params))
                    ...
                    current_loss=my_mse_loss(outputs1, labels1,sigmas1)+constraint_voilations(outputs2)
                    ...
                    loss_constraint_meta_test_tensor.append(constraint_voilations(outputs2))
    ...
    ################ save model ################
    torch.save(model, './pkl/model_meta_'+str(round)+'.pkl') 
