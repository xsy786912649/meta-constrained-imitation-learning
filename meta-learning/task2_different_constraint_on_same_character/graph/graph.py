...
fs_test_cv_sd=np.std(fs_test_cv, axis=0)/2.0
maml_test_cv_mean =  np.mean(maml_test_cv, axis=0)
maml_test_cv_sd=np.std(maml_test_cv, axis=0)/2.0
ours_test_cv_mean =  np.mean(ours_test_cv, axis=0)
ours_test_cv_sd=np.std(ours_test_cv, axis=0)/2.0
...
ax = plt.gca()
plt.plot(axis,fs_test_error_mean,'-',label="Starting from scratch")
...
plt.figure(figsize=(8*1.1,6*1.1))
ax = plt.gca()
plt.plot(axis,fs_test_cv_mean,'-',label="Starting from scratch")
...
plt.figure(figsize=(8*1.1,6*1.1))
ax = plt.gca()
plt.plot(axis,fs_test_cv_pro_mean,'-',label="Starting from scratch")
...
ltext = leg.get_texts()
#plt.setp(ltext, fontsize=18,fontweight='bold') #设置图例字体的大小和粗细
plt.savefig('constraint_voilation_probability_fulshot_nonconvex.pdf') 
plt.show()
